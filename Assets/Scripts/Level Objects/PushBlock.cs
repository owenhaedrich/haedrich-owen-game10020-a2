...
public class PushBlock : MonoBehaviour, ISnapshottable, IPushable
{
    ...
    private bool _pendingSlideStart = false;
    ...
    private void FixedUpdate()
    {
        if (_pendingRestore)
        {
            ...
            _rigidbody.linearVelocity = _snapshotVelocity;

            if (sliding && _snapshotVelocity.sqrMagnitude > 0.001f)
            {
                _pendingSlideStart = true;
            }
        }
        else if (_pendingSlideStart)
        {
            _pendingSlideStart = false;
            StartCoroutine(Slide(_snapshotVelocity.normalized, _snapshotVelocity.magnitude));
        }
    }
    ...
    {
        _snapshotPosition = _rigidbody.position;
        _snapshotVelocity = _rigidbody.linearVelocity;
        _hasSnapshot = true;
    }
